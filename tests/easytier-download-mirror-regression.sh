...
curl() {
  ...
  printf '%s\n' "$url" >>"$attempts"
  printf 'url=%s connect=%s max=%s retry=%s speed_time=%s speed_limit=%s probe=%s\n' "$url" "$connect_timeout" "$max_time" "$retry" "$speed_time" "$speed_limit" "$is_probe" >>"$args_log"
  if (( is_probe == 1 )); then
    return 0
  fi
  printf '%s\n' "$url" >>"$downloads"
...
grep -q "EasyTier 下载策略：mirror-first" <<<"$out"
grep -q "GitHub 下载策略：mirror-first" <<<"$out"
grep -q "当前下载源失败，正在切换下一个源。" <<<"$out"
