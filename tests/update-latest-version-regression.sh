  ...
  case "${SCENARIO:-version}" in
    ...
    redirect)
      ...
      if [[ "$url" == "https://github.com/ike-sh/leikwan-toolkit/releases/latest" && "$effective" == 1 ]]; then
        printf 'https://github.com/ike-sh/leikwan-toolkit/releases/tag/v1.4.16'
        return 0
      fi
      ;;
    tags)
      ...
      if [[ "$url" == "https://api.github.com/repos/ike-sh/leikwan-toolkit/tags" && "$effective" == 0 ]]; then
        printf '[{"name":"v1.4.6"},{"name":"v1.4.16"},{"name":"v1.4.7"},{"name":"not-semver"}]\n' >"$output"
        return 0
      fi
      ;;
    fail)
...
: >"$attempts"
: >"$args_log"
[[ "$(get_latest_release_version)" == "1.4.16" ]]
