...
grep -q "非交互模式，DDNS_AUTO_RESTART_RELAY=true，正在自动重启 relay" <<<"$out_cooldown"
grep -q "relay 最近已自动重启，处于 cooldown，跳过本次自动重启" <<<"$out_cooldown"
...
grep -q "已自动重启 relay" <<<"$out_restart"
