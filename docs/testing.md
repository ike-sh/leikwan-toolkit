...
- `final-menu-regression.sh`：检查 1.4.7 LTS 主菜单只暴露 6 个核心入口，DDNS 和高级维护菜单保持收敛。
...
- `update-latest-version-regression.sh`：检查 latest 可从 API、redirect、tags API 多路径解析，且失败时不输出空 latest。
