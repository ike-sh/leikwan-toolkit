...
Leikwan Toolkit 1.4.9 LTS 保持 Shell 自更新能力。自更新只使用 GitHub Release 包和对应 `.sha256`，不会直接拉取 raw main 分支脚本作为更新产物。
...
1.4.9 起，latest 检测默认使用轻量 `fast` 元数据策略：
...
[INFO] 可设置 LEIKWAN_TARGET_VERSION=1.4.9 后重试。
```
...
```text
hash  leikwan-toolkit-1.4.9.tar.gz
```
