...
Leikwan Toolkit 1.4.12 LTS 保持 Shell 自更新能力。自更新只使用 GitHub Release 包和对应 `.sha256`，不会直接拉取 raw main 分支脚本作为更新产物。
...
1.4.12 起，latest 检测默认使用轻量 `fast` 元数据策略：
...
[INFO] 可设置 LEIKWAN_TARGET_VERSION=1.4.12 后重试。
```

也可以手动指定目标版本并跳过 latest 检测：

```bash
export LEIKWAN_TARGET_VERSION=1.4.12
lq update run
```
...
```text
hash  leikwan-toolkit-1.4.12.tar.gz
```
